				...
				<div class="bootstrap-wizard-1">
					<div class="tab-content mt30">
		                      	...
		                      	<div class="col-xs-3">
									<div class="panel panel-default dsj_photo dsj_photo_xc">
										...
										<c:if test="${empty history }">
											<a href="${ctx }/back/frame/newHouse/auth/newHouse_pictures_detail?newHouseId=${newHouseId }&pictureStatus=${item.id}">
										...
										<c:if test="${!empty history }">
											<a href="${ctx }/back/frame/newHouse/auth/newHouse_pictures_history_detail?newHouseId=${newHouseId }&pictureStatus=${item.id}">
												<c:if test="${item.pictureUrl==null}">
													<img class="dsj_img img-rounded img-responsive" alt="Responsive image" src="${ctx}/static/front/images/newHouse/default.jpg">
												</c:if>
												<c:if test="${item.pictureUrl!=null}">
						...
						<div class="text-center">
		                     <button class="btn btn-primary" type="button" onclick="toAddHouseType('${history }')">下一步</button>
		                </div>
					</div>
				</div>
			</form>
...
<script>
function toAddHouseType(type){
	$("#popup_box").show();
	if(type==''){
		location=_ctx+"/back/frame/newHouse/auth/newHouse_auth_houseTypeList?newHouseId="+$("#newHouseId").val();
	}else{
		location=_ctx+"/back/frame/newHouse/auth/newHouse_auth_history_houseTypeList?newHouseId="+$("#newHouseId").val();
	}
}
</script>