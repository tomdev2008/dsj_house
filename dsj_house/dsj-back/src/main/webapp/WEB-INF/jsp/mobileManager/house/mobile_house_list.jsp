...
<div class="white-head">
			...
			<div class="row">
					<div class="col-xs-12">
						<shiro:hasPermission name='mobileHouse:pc'>
				   		  <button class="dsj_btn btn dsj_btn_green" type="button" onclick="addSmallMobile(1,1)">生成PC端小号</button>
				   		</shiro:hasPermission>
				   		<shiro:hasPermission name='mobileHouse:wap'>
				   		  <button class="dsj_btn btn dsj_btn_green" type="button" onclick="addSmallMobile(1,2)">生成WAP端小号</button>
				   		</shiro:hasPermission>
				   		<shiro:hasPermission name='mobileHouse:app'>
				   		  <button class="dsj_btn btn dsj_btn_green" type="button" onclick="addSmallMobile(1,3)">生成APP端小号</button>
				   		</shiro:hasPermission>
				     </div>
			        <!--表格 S-->
...
var id;
 $(function(){
	 $("body").on("click","#removeBindingModalConfirm",function(){
		 ...
		 $.ajax({
				...
				dataType:"json",
				url:_ctx+"/back/frame/mobileManager/house/removeBinding",
		...
		$("#search_btn").click(function(){
		 ...
		 $('#dataTable').DataTable( {
		     ...
		     /*    "bStateSave": true, */
		      ...
		      "bInfo": true,//页脚信息 
		      "iDisplayLength":10,
		      "sAjaxSource": _ctx+"/back/frame/mobileManager/house/page/list",
		      "language": {
		      "lengthMenu": "",
		      "zeroRecords": "没有找到记录",
		    	   ...
		    	   userManage.getQueryCondition(aoData);
		            ...
		            "aoColumns" : [{"bSortable": false,
		        		...
		        		"aTargets" : [ 3 ]
		                  ...
		                  "mRender":function(data,type,full){
		                	var edit = "<shiro:hasPermission name='mobileHouse:edit'><a class='dsj_btn btn btn-default' href='javascript:void(0);' onclick='editBinding("+full.id+")'>编辑</a></shiro:hasPermission>";
		                	var noBinding = "<shiro:hasPermission name='mobileHouse:removeBing'><a class='dsj_btn btn btn-default' href='javascript:void(0);' onclick='removeBinding("+full.id+")'>解绑</a></shiro:hasPermission>";
		                	var Binding = "<shiro:hasPermission name='mobileHouse:bing'><a class='dsj_btn btn btn-default' href='javascript:void(0);' onclick='binding("+full.id+")'>绑定</a></shiro:hasPermission>";
		                	if(full.status==2){
		                		return Binding;
		                	}else{
 ...
 function binding(mobileId){
	 $("#popup_box").show();
	 location = _ctx+"/back/frame/mobileManager/house/binding?id="+mobileId;
 }
 function editBinding(mobileId){
	 $("#popup_box").show();
	 location = _ctx+"/back/frame/mobileManager/house/editBinding?id="+mobileId;
 }
 ...
 function checkHistory(id){
	 openCommonModal(_ctx+"/back/mobileManager/house/mobile_house_binding_history?id="+id,1000)
 }
 </script>
