						...
						<form class="form-horizontal" id="bindingForm">
						    ...
						    <div class="row">
						        <div class="col-xs-8 col-xs-offset-2">
						            ...
						            <div class="form-group">
						                <label class="col-sm-2 control-label"><span style="color:red">*</span>经纪人名称:</label>
						                <div class="col-sm-6">
						                  	<select id="dicSelectId"  class="form-control dsj-inline js-example-basic-multiple" name="agentId" onchange="setOther()" data-validate="required">
											 </select>
											 <input type="hidden" class="form-control" name="agentName" id="agentName">
						                </div>
						            </div>
						            <div class="form-group">
						                <label for="loupanname" class="col-sm-2 control-label"><span style="color:red">*</span>经纪人编码:</label>
						                <div class="col-sm-6">
						                  <input type="text" class="form-control" name="agentCode" readonly="readonly" id="agentCode">
						                </div>
						            </div>
						            <div class="form-group">
						    ...
						    <div class="row">
						        <div class="col-xs-8 col-xs-offset-2">
						            ...
						            <div class="form-group">
						                ...
						                <div class="col-sm-6">
						                  <input type="text" class="form-control" placeholder="须为手机号,通话未成功时,挂机短信发送到该号码" name="msgPhone" data-validate="required">
						                </div>
						            </div>
						    	</div>
						    </div>
	...
	$("#bindingForm").validate(function (result) {
	  	if(result){
	  		...
	  		$.ajax({
	  			...
	  			dataType:"json",
	  			url:_ctx+"/back/frame/mobileManager/agent/saveBinding",
	  			success:function(resultVo){
	  				if(resultVo.status!=200){
	  					$("#popup_box").hide();
						 setErrorContent(resultVo.message);
					}else{
						location=_ctx+"/back/frame/mobileManager/agent"
					}
	  			}
	  		})
	  	}
	...
	if(houseId!=null){
		for(var i in result){
			if(result[i].id==houseId){
				$("#agentCode").val(result[i].code);
				$("#agentName").val(result[i].text)
