...
(function () {
	 ...
	 * 求时间差
	 */
	Handlebars.registerHelper("timeSrt-T", function(stimeStr, option) {
		var stime = stimeStr;
		var diff = new Date().getTime() - stime;
		var def = "刚刚";
		if (!stime) return def;
	...
	Handlebars.registerHelper('checkedbox', function( curkey , dataStr , options) {
		if(!dataStr) return options.inverse(this);
		var keyArr = dataStr.split(",");
	...
	//内容限制
	Handlebars.registerHelper("contentLimits", function(str , num , option) {
		if(!str) return "";
		var _str ="";
		_str = str.replace('<br/>','').replace('&nbsp;',' ');
		if(_str.length > num ) {
	...
	//内容字数限制判断
	Handlebars.registerHelper("contentLengthLimits", function(str , num , options) {
		if(!str) return "";
		var _str ="";
		_str = str.replace('<br/>','').replace('&nbsp;',' ');
		if(_str.length > num ) {
	 ...
	 * 求时间差
	 */
	Handlebars.registerHelper("isOverTime30", function(stimeStr,options) {
		var stime = stimeStr;
		var diff = new Date().getTime() - stime;
		if(diff >= (1000 * 60 * 30 )){
			return options.fn(this);
	 ...
	 * 手机号名称处理
	 */
	Handlebars.registerHelper("dealPhoneNum", function(str,options) {
		var reg = /^1[0-9]{10}$/;
