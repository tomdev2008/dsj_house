...
public class LoginUserController extends BaseController {
	...
	private static String SEESION_PHONE_VCODE = "warrant_session_phone_vcode";
	...
	public AjaxResultVo sendVcode(String phone, Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		vcode.setType(VerifyCodeTypeEnum.WARRANT_FIND_REGISTER.getValue());
	...
	@ResponseBody
	public AjaxResultVo findPassswordVerify(String phone, String code, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		AjaxResultVo ajax = new AjaxResultVo();
		VerifyCodePo vcode = verifyCodeService.getVerifyByPhoneLast(phone,
				VerifyCodeTypeEnum.WARRANT_FIND_REGISTER.getValue());
	...
	public String toUpdatePassword(String phone, String code, Model model, HttpServletRequest request,
			HttpServletResponse response) {
	...
	@ResponseBody
	public AjaxResultVo updatePassword(String password1, String password2, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		AjaxResultVo ajax = new AjaxResultVo();
		if (StringUtils.isBlank(password1) && StringUtils.isBlank(password2)) {
			...
			if (!password1.equals(password2)) {
				...
				if (session.getAttribute(SEESION_PHONE_VCODE) != null) {
					userService.updatePasswordByPhone(session.getAttribute(SEESION_PHONE_VCODE).toString(), password1,
							UserType.WARRANT.getValue());
