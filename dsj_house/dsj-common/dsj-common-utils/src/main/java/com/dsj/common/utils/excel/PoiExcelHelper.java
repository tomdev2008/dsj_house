...
public abstract class PoiExcelHelper {  
    ...
    /** 获取sheet列表，子类必须实现 */  
    public abstract ArrayList<String> getSheetList(Workbook wb);  
      
    /** 读取Excel文件数据 */  
    public ArrayList<ArrayList<String>> readExcel(Workbook wb, int sheetIndex) {  
        return readExcel(wb, sheetIndex, "1-", "1-");  
    }  
      
    /** 读取Excel文件数据 */  
    public ArrayList<ArrayList<String>> readExcel(Workbook wb, int sheetIndex, String rows) {  
        return readExcel(wb, sheetIndex, rows, "1-");  
    }  
      
    /** 读取Excel文件数据 */  
    public ArrayList<ArrayList<String>> readExcel(Workbook wb, int sheetIndex, String[] columns) {  
        return readExcel(wb, sheetIndex, "1-", columns);  
    }  
      
    /** 读取Excel文件数据，子类必须实现 */  
    public abstract ArrayList<ArrayList<String>> readExcel(Workbook wb, int sheetIndex, String rows, String columns);  
  
    /** 读取Excel文件数据 */  
    public ArrayList<ArrayList<String>> readExcel(Workbook wb, int sheetIndex, String rows, String[] columns) {  
        int[] cols = getColumnNumber(columns);  
          
        return readExcel(wb, sheetIndex, rows, cols);  
    }  
  
    /** 读取Excel文件数据，子类必须实现 */  
    public abstract ArrayList<ArrayList<String>> readExcel(Workbook wb, int sheetIndex, String rows, int[] cols);  
    ...
    private int getColumnNumber(String column) {  
        ...
        for (int i = 0; i < length; i++) {  
            ...
            result += value * Math.pow(26, (double)length - i - 1);  
        }  
        return result - 1;  
    }  
    ...
    public static void main(String[] args) {
    	...
    	PoiExcelHelper helper = getPoiExcelHelper(fileName);  
    ...
    private static void printBody(ArrayList<ArrayList<String>> dataList) {  
        ...
        for(int i=1;i<dataList.size();i++){
        	...
        	for(int j=0;j<data.size();j++){
            	System.out.println(row.get(j)+","+data.get(j)+","+i);	
    ...
    // 获取Excel处理类  
    public static PoiExcelHelper getPoiExcelHelper(String fileName) {  
