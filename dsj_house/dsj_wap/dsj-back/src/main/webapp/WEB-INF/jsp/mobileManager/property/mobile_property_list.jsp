...
<div class="white-head">
			     <form id="searchFromId" class="form-inline">
			    		 ...
			    		 <div class="form-group mr20">
			    		   <span class="wenzi6">代办人名称:</span>
		                    <input type="text" class="small-input form-control" placeholder="代办人名称" name="propertyName">
		                </div>
		          		<div class="form-group mr20">
							<span class="wenzi6">手机号:</span>
		                    <input type="text" class="small-input form-control" placeholder="手机号" name="phone">
		                </div>
		                <div class="form-group mr20">
...
var id;
 $(function(){
	 ...
	 $("body").on("click","#removeBindingModalConfirm",function(){
		 ...
		 $.ajax({
				...
				dataType:"json",
				url:_ctx+"/back/frame/mobileManager/property/removeBinding",
		...
		$("#search_btn").click(function(){
		 ...
		 $('#dataTable').DataTable( {
		     ...
		     /*    "bStateSave": true, */
		      ...
		      "bInfo": true,//页脚信息 
		      "iDisplayLength":10,
		      "sAjaxSource": _ctx+"/back/frame/mobileManager/property/page/list",
		      "language": {
		      "lengthMenu": "",
		      "zeroRecords": "没有找到记录",
		    	   ...
		    	   userManage.getQueryCondition(aoData);
		            ...
		            "aoColumns" : [{"bSortable": false,
		                  "mRender":function(data,type,full){
			                	return "<a href='javascript:void(0);' onclick='checkHistory("+full.id+")'>"+full.mobilePro+"</a>";
			                }
		            },{"bSortable": false,
		                  "mRender":function(data,type,full){
			                	if(full.status==2){
		        		...
		        		"aTargets" : [ 3 ]
		                  ...
		                  "mRender":function(data,type,full){
		                	var edit = "<shiro:hasPermission name='mobileProperty:edit'><a class='dsj_btn btn btn-default' href='javascript:void(0);' onclick='editBinding("+full.id+")'>编辑</a></shiro:hasPermission>";
		                	var noBinding = "<shiro:hasPermission name='mobileProperty:removeBing'><a class='dsj_btn btn btn-default' href='javascript:void(0);' onclick='removeBinding("+full.id+")'>解绑</a></shiro:hasPermission>";
		                	var Binding = "<shiro:hasPermission name='mobileProperty:bing'><a class='dsj_btn btn btn-default' href='javascript:void(0);' onclick='binding("+full.id+")'>绑定</a></shiro:hasPermission>";
		                	if(full.status==2){
		                		return Binding;
		                	}else{
 ...
 function binding(mobileId){
	 $("#popup_box").show();
	 location = _ctx+"/back/frame/mobileManager/property/binding?id="+mobileId;
 }
 function editBinding(mobileId){
	 $("#popup_box").show();
	 location = _ctx+"/back/frame/mobileManager/property/editBinding?id="+mobileId;
 }
 ...
 function checkHistory(id){
	 openCommonModal(_ctx+"/back/mobileManager/property/mobile_property_binding_history?id="+id,1000)
 }
 </script>
