...
<div id="myModal" >
			<div class="modal-header">
                           ...
                           <h4 class="modal-title">编辑租房</h4>
                       </div>
                       <div class="modal-body">
    							 <form id="roleFrom" class="form-horizontal">
							          <div class="form-group">
		                               <label  class="col-sm-2 control-label">租房名称</label>
		                               <div class="col-sm-6">
						                  <select id="dicSelectId"  class="form-control dsj-inline " class="js-example-basic-multiple" >
				 								<option value="${pcRentHousePo.objectId}">${pcRentHousePo.rentName}</option>
										 </select>
						               </div>
						               </div>
							          <div class="form-group">
										    ...
										    <div class="col-sm-9">
										       <input type="text" placeholder="链接地址" value="${pcRentHousePo.linkAddress}" id="linkAddress" class="form-control"  data-validate="required" style="width:400">
										    </div>
							          </div>
							           <div class="form-group">
		                                 <label  class="col-sm-2 control-label">上传图片</label>
						                 <div class="col-sm-6">
							                 <img id="companyLicensePhotoShowImg" style="width: 80px;height: 80px;" src="${pcRentHousePo.picture}"/>
							                 <div id="companyLicensePhotoFilePicker" style="display: inline-block;"> 
							     				 <button class="btn btn-primary" type="button" >选择图片</button>            
							                 </div>
							                 <input value="${pcRentHousePo.picture}" style="width: 0px;border: 0px;" type="text" id="companyLicensePhotoFileUrl"  data-validate="required">
						                 </div>
		                           </div>
							          <input type="hidden" value="${pcRentHousePo.id}" id="pcRentHouseId" > 
							       </form> 
                       </div>
                       <div class="modal-footer">
                            <button type="button" class="dsj_btn btn btn-default" id="close_btn" data-dismiss="modal">取消</button>
                           <button type="button" name="confirm" class="dsj_btn btn dsj_btn_blue" onclick="updateRentHouse()" id="add">确认</button>
                       </div>
                   </div>
   ...
   $(function(){
	   $.fn.modal.Constructor.prototype.enforceFocus =function(){};
		$("#dicSelectId").select2({
				...
				"ajax":{
				    url: _ctx+"/back/rentHouse/general/origin/getDic",
