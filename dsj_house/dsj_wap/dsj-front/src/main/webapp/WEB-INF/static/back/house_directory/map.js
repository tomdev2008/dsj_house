	...
	var map = new BMap.Map("mapBox");//初始化地图
   	var mPoint = new BMap.Point($("#dimension").val(), $("#accuracy").val());
                ...
                for(var j=0;j<results.length;j++){
                    for (var i = 0; i < results[j].getCurrentNumPois(); i ++){  //getCurrentNumPois()获取返回标点个数
                        ...
                        if(results[j].keyword=="地铁站"){
                            pic=_ctx+"/static/front/img/new-house/004.png";
                        }else if(results[j].keyword=="公交站"){
                        	 pic=_ctx+"/static/front/img/new-house/006.png";
                        }else if(results[j].keyword=="火车站"){
                        	 pic=_ctx+"/static/front/img/new-house/004.png";
                        }else if(results[j].keyword=="医疗"){
                        	 pic=_ctx+"/static/front/img/new-house/0013.png";
                        }else if(results[j].keyword=="购物"){
                        	 pic=_ctx+"/static/front/img/new-house/0010.png";
                        }else if(results[j].keyword=="生活"){
                        	 pic=_ctx+"/static/front/img/new-house/009.png";
                        }else if(results[j].keyword=="娱乐"){
                        	 pic=_ctx+"/static/front/img/new-house/0016.png";
                        }else if(results[j].keyword=="科教"){
                        	 pic=_ctx+"/static/front/img/new-house/0016.png";
                        }
                        
					    if(results[j].keyword=="交通"||results[j].keyword=="地铁站"||results[j].keyword=="公交站"||results[j].keyword=="火车站"){
                        	$("#twoMapTab").show();
                        }else{
                        	$("#twoMapTab").hide();
                        }
    ...
    function addMarker(point, pic,index,data){  // 创建图标对象   
        var myIcon = new BMap.Icon(pic, new BMap.Size(30, 80), {    
            // 指定定位位置。   
