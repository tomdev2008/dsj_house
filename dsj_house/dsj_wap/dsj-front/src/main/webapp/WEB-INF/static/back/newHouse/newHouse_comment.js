$.fn.DialogList = function (option) {
	...
	this.objectType = option.object_type;
	...
	this.dataFilter = function (data) {
	    ...
	    this.pageOperation.totalData = data.iTotalDisplayRecords;
	}
		...
		$.ajax({
			...
			success: function(resultVo){
				if(resultVo.status!=200){
					...
					//刷新整页
					location=_ctx+"/front/newHouse/house_comment?id="
						+$("#houseId").val()+"&type="+$("#type").val();
				}
			}
		});
	}
...
function closewindow(self) {
	$("#newsContent").val("");
	$(self).closest(".modal-content").find(".textNum").text("500");
	$(self).closest(".modal-content").find(".delete-pic").click();
