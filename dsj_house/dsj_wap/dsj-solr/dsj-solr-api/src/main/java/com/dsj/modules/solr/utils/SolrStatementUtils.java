...
public class SolrStatementUtils {
	  public static final String NUMBER_PATTERN_1 = "####.0000";
	    ...
	    */
		...
		public static String generateLongRangeMatchStatementEnum(String field, String params,Integer type) {
			Pattern p = Pattern.compile(BusinessConst.STR_TO_STR);
			Matcher m = p.matcher(params);
			if (m.find(0)) {
				params=SolrSynaxConstant.BRACKETS_START+m.group(0).replace("-"," "+ SolrSynaxConstant.TO +" ")+SolrSynaxConstant.BIG_BRACKETS_END;
				   return generateBaseMatchStatement(field, params);
			}
			p = Pattern.compile(BusinessConst.NUMBER_P);
		    m = p.matcher(params);
			if(type==1){
				if (m.find(0)) {
					params=SolrSynaxConstant.BRACKETS_START+SolrSynaxConstant.ANY+SolrSynaxConstant.TO+m.group(0)+SolrSynaxConstant.BIG_BRACKETS_END;
					 return generateBaseMatchStatement(field, params);
				}
			}
			if(type==7){
				if (m.find(0)) {
					params=SolrSynaxConstant.BRACKETS_START+m.group(0)+SolrSynaxConstant.TO+SolrSynaxConstant.ANY+SolrSynaxConstant.BIG_BRACKETS_END;
					 return generateBaseMatchStatement(field, params);
				}
			}
			return "";
		...
		public static String generateLongRangeMatchStatementEnumTwo(String field, String params,Integer type) {
			Pattern p = Pattern.compile(BusinessConst.STR_TO_STR);
			Matcher m = p.matcher(params);
			if (m.find(0)) {
				params=SolrSynaxConstant.BRACKETS_START+m.group(0).replace("-","0000 "+ SolrSynaxConstant.TO +" ")+"0000 "+SolrSynaxConstant.BIG_BRACKETS_END;
				   return generateBaseMatchStatement(field, params);
			}
			p = Pattern.compile(BusinessConst.NUMBER_P);
		    m = p.matcher(params);
			if(type==1){
				if (m.find(0)) {
					params=SolrSynaxConstant.BRACKETS_START+SolrSynaxConstant.ANY+SolrSynaxConstant.TO+Double.parseDouble(m.group(0))*10000+SolrSynaxConstant.BIG_BRACKETS_END;
					 return generateBaseMatchStatement(field, params);
				}
			}
			if(type==7){
				if (m.find(0)) {
					params=SolrSynaxConstant.BRACKETS_START+m.group(0)+"0000"+SolrSynaxConstant.TO+SolrSynaxConstant.ANY+SolrSynaxConstant.BIG_BRACKETS_END;
					 return generateBaseMatchStatement(field, params);
				}
			}
			return "";
		...
		public static void main(String[] args) {
			String params="50m²-70m²";
			Pattern p = Pattern.compile(BusinessConst.STR_TO_STR);
			Matcher m = p.matcher(params);
			if (m.find(0)) {
				params=m.group(0).replace("-"," "+ SolrSynaxConstant.TO +" ");
