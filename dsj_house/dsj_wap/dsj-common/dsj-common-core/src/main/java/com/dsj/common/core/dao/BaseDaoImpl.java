...
public abstract class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public static final String SQL_INSERT_DYNAMIC = "insertDynamic";
	public static final String SQL_BATCH_INSERT = "batchInsert";
	public static final String SQL_UPDATE = "update";
	public static final String SQL_UPDATE_DYNAMIC = "updateDynamic";
	...
	public void setSessionTemplate(SqlSession sessionTemplate) {
	 ...
	 * @return id .
	 */
	public long insertDynamic(T entity) {

		int result = sessionTemplate.insert(getStatement(SQL_INSERT_DYNAMIC), entity);

		if (result <= 0) {
			throw BizException.DB_INSERT_RESULT_0.newInstance("数据库操作,insert返回0.{%s}", getStatement(SQL_INSERT_DYNAMIC));
		}
	...
	public int updateDynamic(T entity) {
		int result = sessionTemplate.update(getStatement(SQL_UPDATE_DYNAMIC), entity);
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,update返回0.{%s}", getStatement(SQL_UPDATE_DYNAMIC));
		}
		return result;
	}
	...
	public int update(List<T> list) {
		...
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,update返回0.{%s}", getStatement(SQL_BATCH_UPDATE));
		}
		return result;
	}
	...
	public PageBean listPage(PageParam pageParam, Map<String, Object> paramMap,String countSql,String sql) {
		...
		paramMap.put("endRowNum", pageParam.getPageNum() * pageParam.getNumPerPage());
		Long count=-1L;
		if(countSql!=null && pageParam.isNeedCount()){
			// 统计总记录数
			count = sessionTemplate.selectOne(getStatement(countSql), paramMap);
		}
		
		// 获取分页数据集
		List<Object> list = sessionTemplate.selectList(getStatement(sql), paramMap);
